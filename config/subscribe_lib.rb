...
def subscribe_album_updated_dj(channel)
  channel.queue('album.updated.dj', durable: true) do |queue| 
...
def subscribe_track_on(channel)
  channel.queue('track.on', durable: true) do |queue| 
    queue.subscribe do |metadata,payload|
      begin
        params = Oj.load(payload)
...
def subscribe_album_resend(channel)
  channel.queue('albums.resend', durable: true) do |queue|
    queue.subscribe do |metadata,payload|
      begin
        params = Oj.load(payload)
...
def subscribe_comment_created_dj(channel)
  channel.queue('comment.created.dj', durable: true) do |queue|
    queue.subscribe do |metadata,payload|
      begin
        params = Oj.load(payload)
...
def subscribe_favorite_created_dj(channel)
  channel.queue('favorite.created.dj', durable: true) do |queue|
    queue.subscribe do |metadata,payload|
      begin
        params = Oj.load(payload)
...
def subscribe_following_destroyed_rb(channel)
  channel.queue('following.destroyed.rb', durable: true) do |queue|
    queue.subscribe do |metadata,payload|
      begin
        params = Oj.load(payload)
...
def subscribe_message_created_dj(channel)
  channel.queue('message.created.dj', durable: true) do |queue|
    queue.subscribe do |metadata,payload|
      begin
        params = Oj.load(payload)
...
def subscribe_relay_created_rb(channel)
  channel.queue('relay.created.rb', durable: true) do |queue|
    queue.subscribe do |metadata,payload|
      begin
        params = Oj.load(payload)
...
def subscribe_audio_queue(channel)
  channel.queue('audio.queue', durable: true) do |queue|
    queue.subscribe do |metadata,payload|
      begin
        params = Oj.load(payload)
...
def subscribe_dig_status_to(channel)
  channel.queue('dig_status.to', durable: true) do |queue|
    queue.subscribe do |metadata,payload|
      begin
        params = Oj.load(payload)
...
def subscribe_update_track_pic_category(channel)
  channel.queue('update.track.pic.category', durable: true) do |queue|
    queue.subscribe do |metadata,payload|
      begin
        params = Oj.load(payload)
...
def subscribe_announcements(channel)
  channel.queue('announcements', durable: true) do |queue|
    queue.subscribe do |metadata,payload|
      begin
        params = Oj.load(payload)
...
def subscribe_user_update_audit(channel)
  channel.queue('user.update.audit', durable: true) do |queue|
    queue.subscribe do |metadata,payload|
      begin
        params = Oj.load(payload)
        args = [ params['uid'],params['nickname'],params['logoPic'],params['intro'],params['create_time'],params['is_update'] ]
...
def subscribe_messages_dj(channel)
  channel.queue('messages.dj', durable: true) do |queue|
    queue.subscribe do |metadata,payload|
      begin
        params = Oj.load(payload)
...
def subscribe_last_uptrack(channel)
  channel.queue('last_uptrack.rb', durable: true) do |queue|
    queue.subscribe do |metadata,payload|
      begin
        params = Oj.load(payload)
...
def subscribe_subapp_created(channel)
  channel.queue('subapp.created', durable: true) do |queue|
    queue.subscribe do |metadata,payload|
      begin
        params = Oj.load(payload)
        app_id = params['id']
...
def subscribe_packapp_feedback(channel)
  channel.queue('packapp.feedback', durable: true) do |queue|
    queue.subscribe do |metadata,payload|
      begin
        params = Oj.load(payload)
